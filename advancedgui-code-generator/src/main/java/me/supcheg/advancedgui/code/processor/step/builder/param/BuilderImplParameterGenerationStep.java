...
public class BuilderImplParameterGenerationStep implements GenerationStep {
    ...
    protected com.palantir.javapoet.FieldSpec.Builder fieldSpec() {
        return FieldSpec.builder(this.parameter.type().box(), this.parameter.name()).addModifiers(Modifier.PRIVATE).addAnnotations(this.fieldAnnotations());
    }
    ...
    protected com.palantir.javapoet.MethodSpec.Builder setter() {
        return MethodSpec.methodBuilder(this.parameter.name()).addAnnotation(NotNull.class).addAnnotation(Override.class).addModifiers(Modifier.PUBLIC).addParameter(ParameterSpec.builder(this.parameter.type(), this.parameter.name(), new Modifier[0]).addAnnotations(this.setterParameterAnnotations()).build()).returns(this.builderImplClass).addCode(this.setterCode());
    }
    ...
    protected com.palantir.javapoet.MethodSpec.Builder getter() {
        return MethodSpec.methodBuilder(this.parameter.name()).addAnnotations(this.getterAnnotations()).addModifiers(Modifier.PUBLIC).returns(this.parameter.type().box()).addCode(this.getterCode());
    }
